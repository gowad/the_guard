...
delete_msg(msg.id, ok_cb, true)
send_large_msg(get_receiver(msg), 'اهلا☺️~ '..msg.from.first_name..'\nيمنع نشر (صور - فيديوهات - صوتيات) و كافة الميديا هنا التزم بقوانين المجموعة  ⛔️🎞 \n👤<b>username</b> :  @'..msg.from.username or '')
return "done"
   end --@h_k_a
   return msg
 end
...
if is_momod(msg) and matches[1]== 'قفل' and matches[2]== 'الروابط بالتحذير' then
   local hka1 = 'hsaif:'..msg.to.id  
    redis:set(hka1, true)
return 'تم تفعيل منع الروابط بالتحذير🔐🌺'
 elseif is_momod(msg) and matches[1]== 'فتح' and matches[2]== 'الروابط بالتحذير' then
   local hka1 = 'hsaif:'..msg.to.id  
    redis:del(hka1)
return 'تم الغاء تفعيل منع الروابط بالتحذير🔓🔑'       
        --@h_k_a
end
