...
if matches[1] == 'قفل' and is_momod(msg) then 
   ...
   local msg_type = 'Audio' 
    ...
    return 'تم تفعيل منع الصوتيات🔐🎤' 
    else 
    return 'منع الصوتيات مفعل مسبقا🔐🎤' 
    end 
   end 
   ...
   local msg_type = 'Video' 
    if not is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type) 
     mute(chat_id, msg_type) 
    return 'تم تفعيل منع الفيديوهات🔐🎬' 
    else 
    return 'منع الفيديوهات مفعل مسبقا🔐🎬' 
    end 
   end 
   ...
   local msg_type = 'Gifs' 
    if not is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type) 
     mute(chat_id, msg_type) 
    return 'تم تفعيل منع الصور المتحركه🔐' 
    else 
    return 'منع الصور المتحركه مفعل مسبقا🔐' 
    end 
            end 
   ...
   local msg_type = 'Documents' 
    ...
    return 'منع الفايلات مفعل مسبقات🔐🗂' 
    end 
            end 
   ...
   local msg_type = 'Text' 
    if not is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type) 
     mute(chat_id, msg_type) 
    return 'المجموعة '..msg.to.title..' الان في وضع الصامت🔕❎' 
    else 
    return 'منع الدردشه مفعل مسبقا🔐' 
    end 
            end 
  ...
  if matches[1] == 'فتح' and is_momod(msg) then 
   ...
   local msg_type = 'Audio' 
    ...
    return 'تم الغاء تفعيل✅ منع الصوتيات🔓🔑' 
    else 
    return 'منع الصوتيات❎ غير مفعل🔓🔑' 
    end 
            end 
   if matches[2] == 'الصور' then 
   local msg_type = 'Photo' 
    if is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
     unmute(chat_id, msg_type) 
    return 'تم الغاء تفعيل ✅ منع الصور🔓🔑' 
    else 
    return 'منع الصور غير مفعل 🔓🔑🌅' 
    end 
            end 
   if matches[2] == 'الفيديو' then 
   local msg_type = 'Video' 
    if is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
     unmute(chat_id, msg_type) 
    return 'تم الغاء تفعيل ✅ منع الفيديوهات🔓🔑🎬' 
    else 
    return 'منع الفيديوهات غير مفعل 🔓🔑🎬' 
    end 
            end 
   if matches[2] == 'المتحركه' then 
   local msg_type = 'Gifs' 
    if is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
     unmute(chat_id, msg_type) 
    return 'تم الغاء تفعيل ✅ منع الصور المتحركه 🔓🔑' 
    else 
    return 'منع الصور المتحركه غير مفعل🔓🔑' 
    end 
            end 
   if matches[2] == 'الفايلات' then 
   local msg_type = 'Documents' 
    if is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
     unmute(chat_id, msg_type) 
    return 'تم الغاء تفعيل ✅ منع الفايلات🔓🔑' 
    else 
    return '🔓🔑قفل الفايلات غير مفعل' 
    end 
            end 
   if matches[2] == 'الدردشه' then 
   local msg_type = 'Text' 
    if is_muted(chat_id, msg_type..': yes') then 
     ...
     unmute(chat_id, msg_type) 
    return ' تم الغاء الصامت 🔔✅ للمجموعة '..msg.to.title..'\n' 
    else 
    return 'قفل الدردشة غير مفعل🔓🔑' 
    end 
            end 
   if matches[2] == 'المجموعه' then 
   local msg_type = 'All' 
    if is_muted(chat_id, msg_type..': yes') then 
     savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
     unmute(chat_id, msg_type) 
    return 'تم الغاء تفعيل ✅ قفل المجموعة🔓🔑' 
                else 
...
--[[ 
   ...
   |_   _|__| |__    / \  | | _| ____|   Dev @li_XxX_il 
     | |/ __| '_ \  / _ \ | |/ /  _|     Dev @h_k_a 
     | |\__ \ | | |/ ___ \|   <| |___    Dev @Aram_omar22 
     |_||___/_| |_/_/   \_\_|\_\_____|   Dev @IXX_I_XXInn 
               CH > @lTSHAKEl_CH 
--]] 
